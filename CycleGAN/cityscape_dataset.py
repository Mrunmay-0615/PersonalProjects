...
class Cityscapes_loader():
    ...
    def __init__(
        ...
        for folder in os.listdir(self.images_base):
            if not (os.path.isdir(os.path.join(self.images_base, folder))):
                continue
            self.files.extend([os.path.join(self.images_base, folder, filename) for filename in os.listdir(os.path.join(self.images_base, folder)) if filename.endswith(".png")])
        if cfg.get('shuffle'):
...
if __name__ == "__main__":
    ...
    cfg = {
        "rootpath" : "Dataset3/Target_CityScapes",
        "split" : "train",
        "img_rows" : 1024,
        "img_cols" : 2048,
        "is_transform": True,
    ...
    dst = Cityscapes_loader(cfg, augmentations=augmentations)
    bs = 4
